...
exports.SQLDumpAll = async (req, res) => {
    try {
        ...
        //-----ADD DATABASE NAME
        const dbName = ["ador", "digiwca", "fped_kiosk", "salesforce_microservice", "product_certificate"]
        ...
        await Promise.resolve(dbName.map((db, i) => {
            ...
            const sqlDumpFolderPath = path.join(__dirname, `../Dump/${db}`, `${date}.sql`)
            if (true) {
                ...
                } else console.log(`Exsists ${date}.sql in ${db} folder `);
            }
        }))
